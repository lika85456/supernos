 ...
 * jackpotBot.send("say Pro více info o jackpotu mi zašeptej '/help'"); toSay =
 ...
 * 
 * if (winnerId == -1) {
 * jackpotBot.send("say Prázdné kolo. Nikdo nevyhrál :(."); continue; } float m
 ...
 * 0; i < jackpotBot.spk.size(); i++) { // spk 1 351883 5 Helèa258 ššš String
 ...
 * " Bohužel se mi nepodaøilo zjistit kolik máš na svém úètì penìz."); } } else
 ...
 * jackpotBot.send("/" + p.get(4) + " " +
 * "V každém kole si beru 7.5% z celkové vsazené èástky."); jackpotBot.send("/"
 * + p.get(4) + " " + "Vyhrané peníze dostaneš pøi další výmìnì.");
 * jackpotBot.send("/" + p.get(4) + " " +
 ...
 * p.get(4) + " " + "/balance - zobrazí tvoje konto"); jackpotBot.send("/" +
 * p.get(4) + " " + "/info - zobrazí informace o právì probíhajícím kole"); //
 * jackpotBot.send("/"+p.get(4)+" "+"/info kolo - // zobrazí informace o kole
 * (pø. /info 1255)"); } } }
 * 
 ...
 * " Byl jsi na 60 sekund zablokován."); } } catch (Exception e) {
 * 
 * }
 * 
 ...
 * .get(jackpotBotTrade.playerID).Name + " právì vsadil " + moneyHeSet); }
 * 
 * } }
 * 
 * }
